...
const DetailsPage = () => {
  ...
  const { data } = useFetchDetails<MovieDetails>(`/${params?.explore}/${params?.id}`) ;
  const { data: castData } = useFetchDetails<CastData>(`/${params?.explore}/${params?.id}/credits`);
  const { data: recommendationData } = useFetch<MovieDetails[]>(`/${params?.explore}/${params?.id}/recommendations`);
              ...
              <div className='container mx-auto px-3 py-16 lg:py-0 flex flex-col lg:flex-row gap-5 lg:gap-10 '>
                  ...
                  <div>
                    ...
                    <div className='grid grid-cols-[repeat(auto-fit,96px)] gap-5 my-4'>
                        ...
                        {
    castData?.cast
        ?.filter((el: CastMember) => el?.profile_path) 
        .map((starCast: CastMember) => { 
            return (
              ...
              {
                playVideo && (
                  <VideoPlay data={{ id: Number(playVideoId) }} close={()=>setPlayVideo(false)} media_type={params?.explore ?? ''}/>
                )
              }
